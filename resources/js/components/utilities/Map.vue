...
defineProps(['singlePlace'])

const initializeMap = component => {
    if (map.value) {
        // ******************* si page détails => affichage d'un seul lieu (pas de pointeur user) *****************
        if (component.singlePlace) {

            const latitude = component.singlePlace.latitude
            const longitude = component.singlePlace.longitude
            ...
            if (userStore.userPosition) {
                // console.log("géoloc déjà acceptée, coordonnées déjà stockées dans le state")

                component.map = L.map('map').setView([userStore.userPosition.latitude, userStore.userPosition.longitude], 13);
                ...
                L.marker([userStore.userPosition.latitude, userStore.userPosition.longitude]).addTo(component.map);
            ...
            if (categories) {
                ...
                categories.forEach(category => { // on boucle sur les catégories 
                    ...
                    component.lieux.forEach(lieu => {  // on boucle sur les lieux et on crée un marqueur pour chaque avec son popup
                        ...
                        if (lieu.category.id == category.id) { // si la catégorie du lieu est bien la catégorie concernée, on crée le marqueur et son pointeur
                            ...
                            let popupContent = "<div class=\"text-center\"><span style=\"display:none\">" + lieu.id + "</span>" +
                                "<div><span class=\"mx-auto fa-2x\" style=\"color:" + lieu.category.couleur + "\">" + lieu.category.icone + "</span>" +
                            ...
                            const fontAwesomeIcon = L.divIcon({   // on crée un icône correspondant à celui choisi pour la catégorie
                                html: `<span style="text-shadow: 2px 2px 4px #fff; color: ${category.couleur}">${category.icone}</span>`,
                                iconSize: [40, 40],
                                className: 'fa-2x ' + category.nom,
                            });

                            L.marker([lieu.latitude, lieu.longitude], { icon: fontAwesomeIcon })
                                .addTo(categoryGroup)   // on ajoute le marqueur au groupe
                    ...
                    category.groupe = categoryGroup

                    // on ajoute le groupe à la map
                    component.map.addLayer(categoryGroup);
                })
